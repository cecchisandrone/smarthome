...
function login (username, password) {
  return new Promise(function (resolve, reject) {
    ...
    axios.post(process.env.VUE_APP_API_ENDPOINT + '/auth', user)
      .then((response1) => {
        axios.get(process.env.VUE_APP_API_ENDPOINT + '/auth/', {headers: { Authorization: `Bearer ${response1.data.token}` }})
          .then((response2) => {
...
function renewToken (token) {
  return new Promise(function (resolve, reject) {
    axios.put(process.env.VUE_APP_API_ENDPOINT + '/auth', {headers: { Authorization: `Bearer ${token}` }})
      .then(function (res) {
