...
<script>
  import ChartCard from '@/components/Cards/StatsCard.vue'
  import * as powerMeterService from '@/services/powerMeterService.js'
  export default {
    components: {
      ChartCard
    },
    ...
    created () {
      ...
      powerMeterService.getScheduledMeasurements().then((data) => {
        ...
        for (const prop in data) {
          ...
          let value = data[prop]
          that.powerMeterChart.data.series[0].push(Math.abs(value))
          that.powerMeterChart.data.series[1].push(value < 0 ? -value : 0)
        }
